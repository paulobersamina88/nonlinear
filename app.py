...
st.set_page_config(page_title="MDOF RSA + Pushover Reconciliation", layout="wide")

st.title("MDOF Response Spectrum + Nonlinear Pushover Reconciliation")
st.caption("STAAD mass/stiffness → modal RSA → first-mode pushover pattern → frame plastic capacity → capacity spectrum / ADRS")
...
**Recommended units:** kN, m, sec. Use mass in **kN·s²/m**. If you have seismic weight in kN, mass = W / 9.80665.

**Important simplification:** storey yield capacity is estimated from plastic moment capacity using a transparent storey mechanism approximation. It is useful for teaching, preliminary checking, and reconciliation, not final certification.
        """
    )

# ------------------------- helpers -------------------------
def default_mass(n):
    return pd.DataFrame({"Storey": np.arange(1, n + 1), "Mass_kNs2_per_m": np.full(n, 250.0)})

def default_k(n):
    vals = np.linspace(140000, 70000, n)
    return pd.DataFrame({"Storey": np.arange(1, n + 1), "Storey_stiffness_kN_per_m": vals})
...
def parse_matrix(text, n, default_matrix):
    ...
    try:
        ...
        st.error(f"Could not parse matrix: {e}")
        return default_matrix

def eig_analysis(M, K):
    w2, phi = eigh(K, M)
    keep = w2 > 1e-9
    w2, phi = w2[keep], phi[:, keep]
    order = np.argsort(w2)
    w2, phi = w2[order], phi[:, order]
    w = np.sqrt(w2)
    T = 2 * np.pi / w
    # normalize roof component positive and = 1 for easy interpretation
...
def storey_capacity(cap_df):
    # V_y = n_frames * mechanism_factor * (sum Mp columns + beam contribution factor * sum Mp beams) / h
    # Mp in kN-m, h in m => kN
    df = cap_df.copy()
    df["Column_component_kN"] = df["Frames_in_axis"] * df["Column_factor"] * df["Sum_column_Mp_per_frame_kNm"] / df["Storey_height_m"]
    df["Beam_component_kN"] = df["Frames_in_axis"] * df["Beam_factor"] * df["Sum_beam_Mp_per_frame_kNm"] / df["Storey_height_m"]
    df["Storey_yield_shear_kN"] = df["Column_component_kN"] + df["Beam_component_kN"]
    return df

def pushover_curve(pattern, k_story, Vy_story, roof_phi, overstrength=1.15, post_ratio=0.05, nsteps=90):
    pattern = np.array(pattern, dtype=float)
    pattern = pattern / np.sum(pattern)
    k_story = np.array(k_story, dtype=float)
    Vy_story = np.array(Vy_story, dtype=float)
    n = len(k_story)
    # storey shear demand from lateral pattern per unit base shear
    ...
    for vb in V:
        ...
        for i in range(n):
            if story_shear[i] <= Vy_story[i]:
                ...
                story_drift[i] = dy + (story_shear[i] - Vy_story[i]) / max(post_ratio * k_story[i], 1e-9)
        roof_delta.append(float(np.sum(story_drift)))
        yielded_count.append(ycount)
    return pd.DataFrame({"Roof_displacement_m": roof_delta, "Base_shear_kN": V, "Yielded_storeys": yielded_count}), V_global_y, critical, shear_factor
...
with col1:
    mass_df = st.data_editor(default_mass(n), num_rows="fixed", use_container_width=True, key=f"mass{n}")
with col2:
    k_df = st.data_editor(default_k(n), num_rows="fixed", use_container_width=True, key=f"k{n}")
...
if input_method == "Full STAAD K matrix":
    ...
    txt = st.text_area("K matrix, kN/m", value="", height=150)
    K = parse_matrix(txt, n, K_default)
else:
    K = K_default
...
st.subheader("Response spectrum input")
...
spec_df = st.data_editor(default_spec, num_rows="dynamic", use_container_width=True, key="spectrum")
...
rsa_storey_shear_srss = np.array([np.sqrt(np.sum([modal_forces[j, i:].sum()**2 for j in range(num_modes)])) for i in range(n)])
...
with c2:
    st.pyplot(plot_xy(spec_df["T_sec"], spec_df["Sa_g"], "Period T, sec", "Sa, g", "Input response spectrum"))

st.header("3. First-mode pushover pattern")
mode_for_pattern = st.selectbox("Pushover pattern source", ["Mode 1 mass × phi", "RSA SRSS floor force shape"], index=0)
if mode_for_pattern.startswith("Mode 1"):
...
pattern_df = pd.DataFrame({"Storey": np.arange(1, n + 1), "Pattern_fraction": pattern, "Pattern_force_for_1kN_base_kN": pattern})
st.dataframe(pattern_df.style.format(precision=5), use_container_width=True)
st.pyplot(plot_xy(pattern, np.arange(1, n + 1), "Lateral force fraction", "Storey", "Pushover lateral load pattern"))

st.header("4. Beam/column plastic moment capacity and number of frames")
st.markdown("Input plastic moment sums **per one frame in the analyzed axis**. The app multiplies by the number of participating frames.")
...
cap_in = st.data_editor(default_cap, num_rows="fixed", use_container_width=True, key=f"cap{n}")
...
with st.expander("Capacity equation used"):
    st.latex(r"V_{y,i}=N_{frames,i}\left(C_c\frac{\sum M_{p,c}}{h_i}+C_b\frac{\sum M_{p,b}}{h_i}\right)")
    st.markdown("Default `Column_factor = 2.0` represents top and bottom column plastic hinges in a storey mechanism. `Beam_factor` is included so you can calibrate whether beam hinges contribute directly to the storey mechanism in your frame idealization.")

st.header("5. Nonlinear pushover curve")
...
with colC:
    target_roof_drift = st.number_input("Target roof drift ratio", 0.001, 0.10, 0.02, 0.001)

push_df, V_y_global, crit_storey, shear_factor = pushover_curve(
    pattern, k_story, cap["Storey_yield_shear_kN"].to_numpy(float), phi[:, 0], overstrength, post_ratio
)
...
st.header("6. ADRS capacity spectrum reconciliation")
...
# demand spectrum converted to displacement
...
ax.plot(demand["Sd_m"], demand["Sa_g"], marker="s", label="Elastic demand spectrum")
...
# crude intersection by nearest Sa difference on Sd grid
if push_df["Sd_m"].max() > 0 and demand["Sd_m"].max() > 0:
    ...
    pp_sd, pp_sa = sd_grid[idx], cap_sa[idx]
    pp_roof = pp_sd * Gamma1 * phi_roof1
    st.success(f"Approximate ADRS performance point: Sd = {pp_sd:.4f} m, Sa = {pp_sa:.3f} g, roof displacement ≈ {pp_roof:.4f} m, drift ≈ {pp_roof/H_total:.3%}")

st.subheader("RSA versus pushover reconciliation summary")
recon = pd.DataFrame({
    "Item": [
        "Total seismic mass", "Mode 1 period", "Mode 1 participation", "Cumulative participation used", 
        "RSA SRSS base shear", "Pushover first-yield base shear", "RSA / yield capacity", "Target roof displacement"
    ],
    "Value": [
        f"{total_mass:,.2f} kN·s²/m", f"{T[0]:.4f} sec", f"{props.loc[0, 'Mass_participation_%']:.2f}%", 
        f"{props.loc[num_modes-1, 'Cumulative_mass_%']:.2f}%", f"{rsa_base_shear_srss:,.1f} kN", 
        f"{V_y_global:,.1f} kN", f"{rsa_base_shear_srss / V_y_global:.2f}", f"{target_roof_drift*H_total:.4f} m"
    ]
})
st.table(recon)
...
st.download_button("Download calculation workbook", data=out.getvalue(), file_name="mdof_pushover_reconciliation.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
